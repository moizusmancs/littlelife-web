...
built too (2026-09-26): the report read by id (a route you added the same day, seeded from the feed's cached copy so opening from the feed costs nothing), its media, the large vote pair, a small map with the distance on request,
...
| `GET /incident-reports/{id}` **(added on request, 2026-09-26 — `backend-requests/incident-report-by-id.md`; same backend branch)** | Incident Detail (later the NGO and admin detail pages) | ✅ Wired (seeded from the feed's cached copy; public; UTC timestamps on this route only) |
...
- **Not built yet, on purpose:** the **Report Incident** button (its drawer is a later screen of this phase). The **card links** to Incident Detail waited for that screen ("links only where the target exists") and were added with it the same day — see *Incident Detail (as built)*.
...
- **Finding the report — first from the list, then by id (both 2026-09-26).** At first there was **no `GET /incident-reports/{id}`**, so the page read the same nationwide list as the feed and found the id in it — free from the feed, but a direct link downloaded every report in the country to show one. That was requested (`supporting-material/backend-requests/incident-report-by-id.md`) and **built the same day**. **As wired now:** `useIncidentReport` reads `GET /incident-reports/{id}`, starting from the feed's cached copy when the list holds the report (dated when the list was fetched, so opening from the feed makes **no request** while it is fresh and never shows a skeleton); a direct link makes **one small request** and never the list. The server's `404` (unknown) and `400` (malformed) are **"Report not found"** — even over a copy cached before the report went — and are not retried; other failures follow the app's own retry setting (the hook first forced one retry, overriding it — found by a unit test); a **rejected** report (the route returns it like any other) shows "This report was rejected" and none of its content. The media list is one `GET …/media` for this report, failing inside its own card. **A vote from the page re-reads only this report**, not the country (the vote hook now also updates the single report's cached copy at once, and re-reads the one-report and list entries on screen).
...
- **Backend gap → requested and built (2026-09-26):** `GET /incident-reports/{id}` — see *Finding the report* above and `supporting-material/backend-requests/incident-report-by-id.md` (✅ built, with its *As built* note).
...
- **Reading by id (added the same day, once `GET /incident-reports/{id}` existed):** probed on :8080 first (see the contract's *As built*). **Unit:** the shared stand-in server gained the route with the backend's rules (`404` unknown, `400` malformed, rejected returned, registered after `my-votes` as in the backend); the page tests now prove a direct link asks for **the one report and never the list**, an unknown and a malformed id are each asked once and read as "not found", a failed read recovers on retry, opening from the feed asks for **neither** the list nor the report, and a vote shows at once (the vote request held by the stand-in) and then re-reads **only this report**. **Mutation-checked:** dropping the feed-copy seed fails the from-the-feed test; dropping the vote's update of the single report fails the voting test (it passed at first because the stand-in answered before the assertion — the request is now held). Not claimed: the "don't retry a 404" rule — the test client never retries, so no test can see it. **E2E:** the direct-link test now watches the network — exactly one `GET /incident-reports/{id}` and **no list request** — and checks the real `404` and `400`; `citizen-incident-detail` **5/5** and `citizen-community` **9/9** (run together, two workers). The page's look didn't change (same parts, new data source), so no new captures.
- **Test data** cleaned (dry run first, every guard 0): 80 accounts, 102 reports, 24 updates removed — and after the by-id runs, 44 accounts, 56 reports, 14 updates; 8 real reports and 5 real updates left. **17 vote rows** remain, not 15: two are yours from 12:51 today (`user1` upvote and `user2` downvote on the "Farmland…" report, cast by hand with the seed accounts) and were left alone; the report's 1/1 matches them.
