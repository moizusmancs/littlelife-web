 ...
 * Owns the two queries the sidebar itself needs — the account's profile (`name` and home region) and the
 * pending-invitation count for the Invitations badge — under shared keys, so Edit Profile's form and the Invitations
...
export function ProfileLayout() {
  ...
  return (
    <div className="flex flex-col gap-6 md:flex-row md:items-start md:gap-8">
      <ProfileSidebar name={data?.name ?? null} homeRegion={data ? homeRegionLabel(data) : null} invitationCount={invitations?.length} />
